...
# Inputs:
#         - (optional) file.std: .csv file for standard walk
#         - (optional) file.ht: .csv file for heel-toe walk
...
mainProcess <- function(file.std = NULL,file.ht = NULL, output.file = NULL, rmSteady = TRUE, rmOdd = TRUE, rmGlitch = TRUE, rotated = TRUE, verbose = FALSE){
  
  
  # If no file is explicitly provided, pop-up chooce file window
  if(length(file.ht) == 0 & length(file.std) == 0 ){ 
    file.std = rchoose.files() #could be multiple files
  }
  
  #now, fixed parameters (because of model training)
  nfreq = 10
  bodysite = 1:25
  ...
  for(f in c(file.std,file.ht)){
    #check if provided file is STD or HT
    if(tail(strsplit(f,'_')[[1]],1) != 'W.csv'){ 
      f.ht = f
      f.std = NULL
    }else{
      f.std = f
    ...
    # search for a standard walk file, if heel-toe has been provided
    if(is.null(f.std)){
      tmp = paste(strsplit(f.ht,'HT')[[1]][-length(strsplit(f.ht,'HT')[[1]])],collapse = 'HT')
      f.std = paste(tmp,'W.csv',sep='')
    }
    # process data only if file exists
    if(file.exists(f.std)){
      x = loadData(f.std)
      ...
      if(verbose) cat('Standard Walk: Fourier analysis done...\n')
      # load predictive model
      load('model/RF_std_model.Rdata')
      # scale new data in the same way than training data
      ...
      #in case of missing data, return 0.5
      preds.std = 0.5
    }
    ...
    # search for a standard walk file, if heel-toe has been provided
    if(is.null(f.ht)){
      tmp =paste(strsplit(f.std,'W')[[1]][-length(strsplit(f,'W')[[1]])],collapse = 'W')
      f.ht = paste(tmp,'HT.csv',sep='')
    }
    ...
    if(file.exists(f.ht)){
      ...
      if(verbose) cat('HT walk: Fourier analysis done...\n')
      # close graphical output
      graphics.off()
      
      # load predictive model
      load('model/RF_ht_model.Rdata')
      # scale new data in the same way than training data
    ...
    load('model/ensemble_model.Rdata')
    fitted.results <- predict(m,newdata=data.frame('std_score'=preds.std,'ht_score' = preds.ht),type='response')
    
    #store output
    OUTPUT = rbind(OUTPUT,c(paste(strsplit(f,'_')[[1]][-length(strsplit(f,'_')[[1]])],collapse='_'),preds.std,preds.ht,fitted.results))
  }
  colnames(OUTPUT) = c('individual','standard_walk_score','heel_toe_score','hybrid_score')
  
  if(is.null(output.file)){
    print(unique(OUTPUT))
  }else{
    write.csv(unique(OUTPUT),file=output.file,quote = FALSE,row.names = FALSE)
  }
  #output scores
